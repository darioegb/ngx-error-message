...
describe('WithoutInternationalizationComponent', () => {
  ...
  it('should return form controls, name controls, and aliases', () => {
    ...
    expect(component.nameControls).toBe(
      (component.formControls['name'] as FormGroup).controls,
    )
    expect(component.aliases.length).toBe(
      (component.formControls['aliases'] as UntypedFormArray).length,
    )
  })
